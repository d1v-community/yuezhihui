...
import { fetchHealthScoreDetailForUser } from "~/services/analysis.server";
...
export async function loader({ request }: LoaderFunctionArgs) {
  try {
    ...
    const data = await fetchHealthScoreDetailForUser(user.id, q.limit);
    ...
    const msg = e?.message || "获取健康分数明细失败";
