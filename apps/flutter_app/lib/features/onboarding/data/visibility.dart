...
bool isQuestionVisible(String id, Map<String, dynamic> answers) {
  ...
  switch (id) {
    ...
    case 'H5_abortion_history':
      ...
      return menarcheEver == 'yes' &&
          (pregnancyHistory == 'ever' || pregnancyHistory == 'pregnant_now');
    ...
    case 'M1_pregnancy_possible':
      if (!(menarcheEver == 'yes' && currentStatus != 'menopause')) {
        return false;
      }
      return mensesLast3m == 'no' ||
          _includesMulti(answers, 'F1_health_conditions', '非经期出血');
...
List<String> getVisibleQuestionIds(Map<String, dynamic> answers) {
  return onboardingQuestionOrder
      .where((id) => isQuestionVisible(id, answers))
      .toList();
}
